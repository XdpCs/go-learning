...
func main() {
	...
	// 为了使用解码 map 中的值，需要将他们进行适当的类型转换
	// 例如，这里将 num 的值转换成 float64 类型
	num := dat["num"].(float64)
	fmt.Println(num)
	...
	// 可以将 JSON 解码为自定义数据类型
	// 这样做的好处是，可以为程序增加附加的类型安全性， 并在访问解码后的数据时不需要类型断言
	...
	// 当然，也可以像 os.Stdout 一样直接将 JSON 编码流传输到 os.Writer 甚至 HTTP 响应体
